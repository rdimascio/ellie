...
test('private state has restricted permissions and credentials are stored only as hashes', async () => {
  ...
  try {
    ...
    const controller = newToken(); await Auth.initialize(controller, dir);
    const auth = await Auth.open(dir); const invite = await auth.invite();
    const nodeToken = await auth.pair(invite.code, 'test-node');
    const disk = await readFile(join(dir, 'auth.json'), 'utf8');
    assert.ok(!disk.includes(controller) && !disk.includes(nodeToken) && !disk.includes(invite.code));
...
test('config validates transport and version before use', () => {
  assert.throws(() => serverConfig({ version: 1, host: 'localhost', port: 70000, preferences: defaults }));
  assert.throws(() => nodeConfig({ version: 1, id: 'test', serverUrl: 'http://localhost', preferences: defaults }));
  assert.throws(() => nodeConfig({ version: 2, id: 'test', serverUrl: 'https://localhost', preferences: defaults }));
});
test('pairing is single-use even under concurrency, and invitation replacement invalidates the old code', async () => {
  ...
  const stale = await auth.invite(); const invite = await auth.invite();
  await assert.rejects(auth.pair(stale.code, 'stale'));
  const results = await Promise.allSettled([auth.pair(invite.code, 'first'), auth.pair(invite.code, 'second')]);
  assert.equal(results.filter(r => r.status === 'fulfilled').length, 1);
});
test('expired invitation is rejected', async () => {
  const auth = new Auth({ identities: [], invitation: { hash: 'not-a-valid-hash', expiresAt: Date.now() - 1 } }, async () => {});
