  ...
  try {
    ...
    const retainedEvidence =
      mode === "retained"
        ...
        : undefined;
    outcome = { diagnostic, result, retainedDerived, retainedEvidence };
  } catch (error) {
...
test("early process failure preserves a diagnostic without an xcresult", async () => {
  ...
  assert.match(
    diagnostic,
    /^iOS runner diagnostic: stage=simulator-discovery outcome=exit-status stageMs=\d+ totalMs=\d+ xcode=not-started result=not-created cleanup=derived-removed\n$/,
  );
});
