...
struct EllieApp: App {
  ...
  init() {
    ...
    _agendaStore = StateObject(
      wrappedValue: AgendaStore(
        fileURL: stateURL?.deletingLastPathComponent().appendingPathComponent("agendav1.json")))
  }

  var body: some Scene {
    WindowGroup("Ellie") {
      DashboardView(
        store: store, choresStore: choresStore, weatherStore: weatherStore,
...
@MainActor enum AgendaFiles {
  static func importFile(into store: AgendaStore) {
    ...
    panel.title = "Import agenda snapshot"
    panel.allowedContentTypes = [.json]
    panel.allowsMultipleSelection = false
    panel.canChooseDirectories = false
    guard panel.runModal() == .OK, let url = panel.url else { return }
    do { store.importData(try AgendaStore.safeRead(url)) } catch {
      store.message = "Choose a regular Ellie agenda JSON file smaller than 128 KB."
    }
  }
}
  ...
  static func importFile(into store: DashboardStore) {
    ...
    do {
      ...
      confirmation.informativeText =
        "Importing replaces all dashboards on this Mac. Export a copy first if you want to keep the current layouts and notes."
      ...
      store.error =
        "This file could not be opened as an Ellie dashboard export. Your dashboards are unchanged."
    }
  }
