...
struct DashboardView: View {
    ...
    var body: some View {
        NavigationSplitView {
            VStack(spacing: 0) {
                    ...
                    Section("Dashboards") {
                        ForEach(store.state.dashboards) { dashboard in
                            Label(dashboard.name, systemImage: dashboard.id == store.state.dashboards.first?.id ? "house" : "rectangle.3.group")
                                .tag(dashboard.id)
                                .contextMenu {
                                    Button("Rename…") { store.selectedID = dashboard.id; name = dashboard.name; renaming = true }
                                    Button("Delete…", role: .destructive) { store.selectedID = dashboard.id; deleting = true }
                ...
                .safeAreaInset(edge: .bottom) {
                    HStack {
                        Button { name = ""; newDashboard = true } label: { Label("New dashboard", systemImage: "plus") }
            ...
            if let dashboard = store.selectedDashboard {
                ScrollView {
                    VStack(alignment: .leading, spacing: 30) {
                        HStack(alignment: .bottom) {
                            ...
                            Spacer()
                            if editing {
                                Button("Rename…") { name = dashboard.name; renaming = true }
                                    .buttonStyle(.borderless)
                            }
                        }
                        if dashboard.widgets.isEmpty {
                            ...
                            } description: { Text("Add a clock, a note, or a place for what comes next.") }
                            actions: { Button("Add a widget") { gallery = true } }
                            .frame(minHeight: 320)
                        } else {
                ...
                ContentUnavailableView("Create your first dashboard", systemImage: "rectangle.3.group", description: Text("Use the plus button in the sidebar to begin."))
            }
        }
        ...
        .alert("New dashboard", isPresented: $newDashboard) {
            ...
            Button("Create") { store.createDashboard(name: name) }.disabled(name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
        }
        ...
        .confirmationDialog("Delete this dashboard?", isPresented: $deleting, titleVisibility: .visible) {
            Button("Delete dashboard", role: .destructive) { if let id = store.selectedID { store.deleteDashboard(id: id) } }
        } message: { Text("Its widgets and notes will be removed from this Mac. Export a copy first if you want to keep them.") }
    ...
    private func widgetRows(_ dashboard: Dashboard, columns: Int) -> some View {
        ...
        return VStack(spacing: 18) {
            ForEach(Array(rows.enumerated()), id: \.offset) { _, row in
                HStack(alignment: .top, spacing: 18) {
                    ForEach(row) { widget in
                        NativeWidgetCard(widget: widget, choresStore: choresStore, openChores: { showingChores = true }, editing: editing, weatherStore: weatherStore,
            ...
            if editing {
                ...
                .buttonStyle(.borderless)
                .background(RoundedRectangle(cornerRadius: 18).strokeBorder(.quaternary, style: StrokeStyle(lineWidth: 1, dash: [5, 5])))
            }
        }
    }
    ...
    private func makeRows(_ widgets: [DashboardWidget], columns: Int) -> [[DashboardWidget]] {
        ...
        for widget in widgets {
            if columns == 2 && widget.size == .small, let last = rows.last, last.count == 1, last[0].size == .small {
                rows[rows.count - 1].append(widget)
            } else { rows.append([widget]) }
        }
        return rows
    }
}
