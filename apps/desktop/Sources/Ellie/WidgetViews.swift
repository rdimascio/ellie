  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 22) {
      HStack(spacing: 8) {
        ...
        if editing {
          HStack(spacing: 8) {
            Button(action: earlier) { Image(systemName: "arrow.left") }.disabled(isFirst).help(
              "Move earlier"
            ).accessibilityLabel("Move \(widget.title) earlier")
            Button(action: later) { Image(systemName: "arrow.right") }.disabled(isLast).help(
              "Move later"
            ).accessibilityLabel("Move \(widget.title) later")
            Button(action: configure) { Image(systemName: "slider.horizontal.3") }.help(
              "Edit \(widget.title)"
            ).accessibilityLabel("Edit \(widget.title)")
            Button(role: .destructive, action: remove) { Image(systemName: "minus.circle") }.help(
              "Remove \(widget.title)"
      ...
      Group {
        if isClock {
          NativeClock(timeZone: widget.config["timeZone"], wide: widget.size == .wide)
        } else if isNote {
    ...
    .overlay(
      RoundedRectangle(cornerRadius: 22, style: .continuous).strokeBorder(
        Color.primary.opacity(isClock ? 0 : 0.04), lineWidth: 1)
    )
    .contextMenu {
  ...
  private var background: Color {
    ...
    if isNote {
      return colorScheme == .dark
        ? Color(red: 0.22, green: 0.19, blue: 0.13) : Color(red: 1, green: 0.96, blue: 0.82)
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
          ...
          Text(
            zone.identifier == TimeZone.current.identifier
              ? "Local time" : zone.identifier.replacingOccurrences(of: "_", with: " ")
          )
          .font(.system(size: 12)).foregroundStyle(.white.opacity(0.55))
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        if wide {
          ClockDial(date: context.date, timeZone: zone).frame(width: 138, height: 138)
...
private struct ClockDial: View {
  ...
  var body: some View {
    ...
    ZStack {
      ...
      Capsule().fill(.white).frame(width: 3, height: 34).offset(y: -14).rotationEffect(
        .degrees(hour * 30 + minute / 2))
      Capsule().fill(.white).frame(width: 2, height: 48).offset(y: -22).rotationEffect(
        .degrees(minute * 6))
      Capsule().fill(Color.orange).frame(width: 1, height: 57).offset(y: -22).rotationEffect(
        .degrees(second * 6))
          ...
          Button {
            ...
            VStack(alignment: .leading, spacing: 12) {
              HStack {
                ...
                Spacer()
                Image(systemName: "plus.circle.fill").font(.system(size: 18)).foregroundStyle(
                  .tertiary)
              }
...
struct WidgetInspector: View {
  ...
  init(
    widget: DashboardWidget, weatherStore: WeatherStore, agendaStore: AgendaStore,
    save: @escaping (String, WidgetSize, [String: String]) -> Void
  ) {
    self.widget = widget
      ...
      Form {
        ...
        if widget.type == .weather {
          Section("Forecast") {
            ...
            if weatherEnabled {
              ...
              Text(
                "Ellie sends these coordinates to Open-Meteo only when enabled. It never requests your Mac’s location."
              )
              .font(.caption).foregroundStyle(.secondary)
              ...
              Text(
                "Weather stays entirely offline. Turning this off deletes the saved place and forecast."
              )
              .font(.caption).foregroundStyle(.secondary)
            }
        ...
        if widget.type == .playlist {
          Section("YouTube playlist") {
            TextField(
              "Playlist URL or ID", text: $playlistInput,
              prompt: Text("https://www.youtube.com/playlist?list=…"))
            Text(
              "Use a public playlist whose ID begins with PL. Ellie contacts YouTube only when you press Play."
            )
            .font(.caption).foregroundStyle(.secondary)
            Text(
              "Playback uses YouTube’s privacy-enhanced embedded player without your account cookies."
            )
            .font(.caption).foregroundStyle(.secondary)
          }
        }
        if widget.type == .calendar {
          Section("Agenda snapshot") {
            Text(
              "Import a read-only Ellie agenda JSON snapshot. It is stored privately on this Mac and applies to every calendar widget."
            ).font(.caption).foregroundStyle(.secondary)
            Button("Import Snapshot…") { AgendaFiles.importFile(into: agendaStore) }
            if agendaStore.canClear {
              Button("Disconnect and Clear", role: .destructive) { agendaStore.disconnect() }
            }
            if let message = agendaStore.message {
              Text(message).font(.caption).foregroundStyle(.red)
            }
          }
        }
      ...
      HStack {
        ...
        Button("Save") {
          ...
          if widget.type == .weather {
            if weatherEnabled {
              guard
                weatherStore.configure(
                  name: placeName, latitudeText: latitude, longitudeText: longitude)
              else { return }
            } else if !weatherStore.disable() {
              return
            }
          }
  ...
  private var validWeatherPlace: Bool {
    ...
    guard !name.isEmpty, name.utf16.count <= 80,
      let latitude = Double(latitude), let longitude = Double(longitude)
    else { return false }
        ...
        if let snapshot = store.snapshot {
          ...
          ForEach(Array(upcoming)) { event in
            HStack(alignment: .top, spacing: 10) {
              RoundedRectangle(cornerRadius: 2).fill(Color.secondary.opacity(0.4)).frame(
                width: 3, height: 34)
              VStack(alignment: .leading, spacing: 2) {
                Text(event.title).font(.system(size: 14, weight: .medium)).lineLimit(1)
                Text(detail(event, snapshot)).font(.caption).foregroundStyle(.secondary).lineLimit(
                  1)
              }
            }
          }
          Spacer(minLength: 0)
          Text(store.freshness(at: context.date)).font(.caption2).foregroundStyle(.secondary)
        } else {
          Image(systemName: "calendar.badge.plus").font(.system(size: 32, weight: .light))
            .foregroundStyle(.secondary)
          Spacer()
          Text("Import an agenda").font(.system(size: 19, weight: .semibold))
          Text("Calendar stays offline until you choose a read-only snapshot.").font(.caption)
            .foregroundStyle(.secondary)
    ...
    Group {
      if !store.state.enabled {
        VStack(alignment: .leading, spacing: 13) {
          Image(systemName: "location.slash").font(.system(size: 32, weight: .light))
            .foregroundStyle(.secondary)
          Spacer(minLength: 0)
          Text("Choose a place").font(.system(size: 19, weight: .semibold))
          Text("Weather is off until you add coordinates and enable Open-Meteo.").font(
            .system(size: 12)
          ).foregroundStyle(.secondary)
        ...
        VStack(alignment: .leading, spacing: 10) {
          HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 2) {
              Text(place.name).font(.system(size: 13, weight: .medium)).foregroundStyle(.secondary)
                .lineLimit(1)
              Text("\(Int(snapshot.temperature.rounded()))°").font(.system(size: 54, weight: .thin))
                .tracking(-2)
            }
            Spacer()
            Image(systemName: snapshot.symbol).symbolRenderingMode(.multicolor).font(
              .system(size: 42))
          }
          Text(snapshot.condition).font(.system(size: 18, weight: .medium))
          Text(
            "Conditions as of \(snapshot.observedAt.formatted(date: .omitted, time: .shortened))"
          )
          .font(.caption).foregroundStyle(.secondary)
          Text(
            "Feels like \(Int(snapshot.apparentTemperature.rounded()))°  ·  Wind \(Int(snapshot.windSpeed.rounded())) mph"
          )
          .font(.system(size: 12)).foregroundStyle(.secondary)
          Spacer(minLength: 0)
          TimelineView(.periodic(from: .now, by: 60)) { context in
          ...
          Link(
            "Weather data by Open-Meteo.com", destination: URL(string: "https://open-meteo.com/")!
          )
          .font(.system(size: 10))
        }
      } else {
        VStack(alignment: .leading, spacing: 13) {
          ...
          Text(store.message ?? "Fetching the first forecast…").font(.system(size: 12))
            .foregroundStyle(.secondary)
