...
interface State {
  identities: Identity[];
  invitation?: { hash: string; expiresAt: number };
}
export class Auth {
  private state: State;
  private persist: (state: State) => Promise<void>;
  private lock: Promise<void> = Promise.resolve();
  constructor(state: State, persist: (state: State) => Promise<void>) {
    this.state = state;
    this.persist = persist;
  }
  static async open(dir?: string): Promise<Auth> {
    return new Auth(await load<State>("auth.json", dir), (state) => save("auth.json", state, dir));
  }
  static async initialize(controller: string, dir?: string): Promise<void> {
    await save(
      "auth.json",
      { identities: [{ id: "controller", role: "controller", tokenHash: hash(controller) }] },
      dir,
    );
  }
  ...
  async pair(code: string, id: string): Promise<string> {
    return this.mutate(async () => {
      ...
      if (!invitation || invitation.expiresAt < Date.now() || !equal(invitation.hash, hash(code)))
        throw new Error("Pairing code is invalid or expired.");
      if (this.state.identities.some((identity) => identity.id === id))
        throw new Error("Node ID already exists.");
      const token = newToken();
      const next: State = {
        identities: [...this.state.identities, { id, role: "node", tokenHash: hash(token) }],
      };
      await this.persist(next);
      this.state = next;
      return token;
  ...
  async revoke(id: string): Promise<void> {
    return this.mutate(async () => {
      const next = {
        ...this.state,
        identities: this.state.identities.filter(
          (identity) => identity.role !== "node" || identity.id !== id,
        ),
      };
      await this.persist(next);
      this.state = next;
