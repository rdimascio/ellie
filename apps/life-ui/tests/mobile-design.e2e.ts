...
export async function runMobileDesignAcceptance({
  launchBrowser = () => browserType.launch({ headless: true }),
  startPreview = startDesignPreview,
}: {
  ...
  try {
    ...
    await page.route("**/api/life/records/design-finance-insight", (route) =>
      route.fulfill({ status: 503, json: { message: "Record detail temporarily unavailable" } }),
    );
    await page.locator(".bottom").getByRole("button", { name: "Finances", exact: true }).click();
    await expect(page.locator(".finance-accounts")).toContainText("Everyday account");
    await expect(page.locator(".finance-page").getByRole("status")).toContainText(
      "Some insights couldn’t be verified",
    );
    await expect(page.locator(".finance-insights")).toHaveCount(0);
    await context.close();
    console.log(
...
if (import.meta.main) await runMobileDesignAcceptance();
