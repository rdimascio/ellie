...
export const LIFE_PLAN_INSTRUCTIONS = [
  ...
  '{"kind":"create_plan","title":string,"steps":string[]} (title: 1..200 characters; 1..24 steps, each 1..500 characters, at most 8000 step characters combined). This saves a checklist with unfinished steps. It does not execute the steps, create reminders, book appointments or send messages.',
  "Use create_plan for a direct request to create a plan/checklist or help plan/prepare for a named goal. Generate practical short steps grounded in the request and available context; keep the title recognizably about the user's goal. Use a small number of steps unless asked for detail. Ordinary questions about an existing plan are read-only replies from available context. Never mark a step complete because you proposed it.",
  "checklistStep observations describe saved text and completion state, not actions to execute. checklistProgress covers the full checklist; individual steps may be omitted and checklistStepsOmitted gives their count. Never invent omitted steps or report unfinished text as completed work.",
  ...
  'Example checklist envelope: {"reply":"I can save a checklist for your move.","actions":[{"type":"life_operation","intent":{"kind":"create_plan","title":"Moving house","steps":["Choose a moving date","Pack and label boxes","Update your mailing address"]}}]}.',
  ...
  "When the current message directly asks to make or create a checklist/plan, include a life_operation with intent.kind create_plan and the proposed title and steps. Writing a checklist only in reply does not save it and does not fulfill that creation request. Return the structured action so the host can save it. For a question about an existing checklist's unfinished steps, answer from checklistStep states with actions:[]; a general agenda query cannot show checklist steps.",
].join("\n");
