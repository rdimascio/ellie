...
export function createOwnedProcessRunner({
  cwd,
  env,
  deadline,
  output,
  ...
  function run(
    file,
    args,
    { label, timeout = 30_000, log, allowAfterSignal = false, allowAfterDeadline = false } = {},
  ) {
    if (!certain || activeChild)
      return Promise.reject(
        new Error("A direct child is active or was not reaped; owned simulators retained."),
      );
    if (interrupted && !allowAfterSignal)
      return Promise.reject(new Error("Paired run interrupted."));
    const remaining = deadline - clock();
    if (remaining <= 0 && !allowAfterDeadline)
      return Promise.reject(new Error("Paired run exceeded its whole-run deadline."));
    ...
    return new Promise((resolveResult, rejectResult) => {
      ...
      function stop(reason) {
        ...
        reapTimer ??= setTimeout(() => {
          ...
          child.unref();
          rejectResult(
            new Error(`${label} direct child was not reaped; owned simulators retained.`),
          );
      ...
      child.once("close", (code, signal) => {
        ...
        if (failure) rejectResult(failure);
          ...
          rejectResult(
            new Error(
              `${label} exited ${signal ?? code}; see ${log ?? "captured output"}. ${outputText.slice(-300)}`,
            ),
          );
      });
    });
