...
function runner(overrides = {}) {
  return createOwnedProcessRunner({
    cwd: process.cwd(),
    env: process.env,
...
test("missing executable closes its owned child and leaves command lane usable", async () => {
  ...
  await assert.rejects(
    commands.run("/definitely/missing/watch-paired-child", [], { label: "missing" }),
    /could not start/,
  );
  assert.equal(commands.certain, true);
  assert.equal(commands.active, false);
  assert.equal(
    await commands.run(process.execPath, ["-e", "process.stdout.write('ok')"], { label: "next" }),
    "ok",
  );
});
...
test("timeout escalates to the retained direct child and waits for its close", async () => {
  ...
  assert.equal(commands.active, false);
  assert.equal(
    await commands.run(process.execPath, ["-e", "process.stdout.write('reaped')"], {
      label: "next",
    }),
    "reaped",
  );
...
test("signal requests the same finite stop path", async () => {
  ...
  assert.ok(
    ...
    "signal must not wait for the original command timeout",
  );
...
test("unreaped direct child permanently blocks cleanup commands", async () => {
  ...
  await assert.rejects(
    commands.run("must-not-spawn", [], { label: "overlap" }),
    /active or was not reaped/,
  );
  await assert.rejects(pending, /not reaped/);
  assert.deepEqual(signals, ["SIGTERM", "SIGKILL"]);
  assert.equal(commands.certain, false);
  await assert.rejects(
    commands.run("must-not-spawn", [], {
      label: "cleanup",
