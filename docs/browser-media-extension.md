...
The source is `apps/browser-media-extension`. Its production manifest requests `activeTab`, `scripting`, and local `nativeMessaging`. Its code permits the exact HTTPS origins `www.netflix.com`, `www.youtube.com`, `tv.youtube.com`, and `www.disneyplus.com`; there are no persistent all-site permissions, website-to-native HTTP server, browser-cookie reads or household credentials.
...
All four production origins remain experimental. A synthetic catalogue passing tests does not establish authenticated service control. The Netflix title adapter recognizes bounded same-origin title/video links and scrollable row containers; it does not cover every service layout, carousel, overlay or player. The public Netflix landing page does not provide authenticated playback. The separate YouTube TV companion reports only a gated page or a single observed player and permits vertical scroll or play/pause after a fresh read; search, title selection and horizontal rows are unavailable. It does not identify which program a video contains. On an exact `/browse/entity-<UUID>` page, the Disney+ companion reads visible title links, permits one vertical viewport scroll when the observed and current page each have the same unambiguous scroller, and selects a title only from a subsequent fresh read. Search, player actions and profile/subscription flows are unavailable. Public title navigation does not establish playback. No DRM, authentication or subscription behavior is bypassed.
