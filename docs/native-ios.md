...
As of 2026-09-13, this repository implements the protocol, server lifecycle, controller CLI and the scoped iOS pairing client. Command dispatch from the iPhone remains a later slice. Enrollment still requires a controller-issued invitation and explicit confirmation; the app does not create household authority or reuse browser, node or controller credentials.
