...
The ATS runner also hosts the production native Life session and account routes with synthetic Calendar and Gmail providers. Its app-hosted tests use the real pinned iOS clients to read one selected calendar and explicitly selected message bodies, then check separate Life grant denial and late cancellation, account-change, and revocation responses. The local `node --test tests/ios-google-life-https.test.ts` check covers the same production HTTPS gateway with synthetic data. These checks do not perform Google OAuth, access a real mailbox or calendar, or validate a physical iPhone.
