...
Remote Life may inspect existing connected account status and use existing authorized account controls. It does not start Google authorization from a remote WebView. The desktop Google flow needs a host loopback callback and an external system browser; using the remote HTTPS origin as that redirect is not equivalent. With the Google desktop client configured for the current coordinator Life instance, the local owner runs `bun run ellie life settings` on that Mac to open its Connected accounts page. The controller response and normal command output contain no launch URL, token or account credential. Remote account connection start/callback remains rejected. A proper cross-device OAuth handoff is a later integration.
...
The release owner's independent review of `18ade2772239a56b6688f81b35ae4dd7d47353a1` identified a missing port comparison in the WebKit server-trust delegate. The delegate now requires HTTPS, the exact enrolled host and its effective port (443 when omitted) before the existing certificate pin and validity evaluation. This also covers subresource challenges that do not pass through navigation policy. A focused endpoint test covers implicit/explicit 443, the configured custom port, wrong ports including zero, another host and non-HTTPS endpoints. There are now nine `LifeWebSessionTests` cases.

The release owner reported that Xcode 16.2 on the MacBook could not compile the original `.strict` cookie assertion; zero of the original eight tests executed. The assertion now compares `sameSitePolicy?.rawValue` with Foundation's canonical `"strict"` value, preserving its meaning without that unavailable member. The same original source passed a generic signing-disabled iOS Simulator build; no Simulator boot, app installation or physical phone acceptance was performed. Original remote logs remain under `/private/tmp/ellie-life-review.tgVogy` on the MacBook: `life-swift.log` SHA-256 `372218edf274056d6fff6ea2406c99467a3cdff22534e363eeebc23ef8550d50` and `ios-build.log` SHA-256 `1064f1376aeda69e3087f2006d317ca50fdaee30c5e9f587634aff977445db7d`.

Follow-up local validation used Command Line Tools only: `swiftc -frontend -parse` passed for the changed production and test files; a temporary Foundation probe executed the actual endpoint test's ten assertions against the extracted production helper and checked the unchanged production cookie construction for Strict, Secure and HttpOnly behavior. The probe passed (`/tmp/ellie-life-native-policy-followup.log`), and `git diff --check` passed. Astra independently reviewed the narrow trust-policy change. No full union gate or Xcode run was duplicated here; the release owner must rerun all nine native tests and validate the combined candidate. No live credentials, services or preview data were used.
