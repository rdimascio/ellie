...
The first slice is in progress on `codex/node-hardening-validation`. The remaining slices are a reviewed design direction, not implemented or accepted production behavior. No Team ID or signing credential is checked into the public repository.
