...
The Netflix native companion path now binds only an explicitly selected, foreground `www.netflix.com` tab through the existing authenticated native host. A fresh read exposes bounded same-origin title/watch candidates and an observed player state. Vertical scrolling, one observed-title selection, and play/pause require the current binding and read; horizontal scrolling is admitted only when exactly one visible scrollable row is identified. Each mutation consumes its observation and reports an unknown outcome until a separate refresh/read. Netflix search, profile/account/payment controls, ambiguous rows or players, and unreviewed pages fail closed. The synthetic extension and Node tests establish this local contract, not real Netflix account or playback acceptance; the latter needs an attended owner session. YouTube TV and Disney+ remain denied.
