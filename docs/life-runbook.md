...
On an enrolled iPhone with an explicit `life.account` grant, the Calendar dashboard widget can select one authorized Google Calendar connection and read its already imported upcoming events. Its **Refresh** reads the host's existing selected-calendar evidence in the phone's display time zone; it does not call Google or start a provider sync. The widget labels the last import and read times, keeps a failed read's rows only as cached data in the current app session, and requires a new read after relaunch. Removing the account or revoking access clears private rows. The Calendar and Gmail views can open Ellie Life to inspect connection status, but Google consent, provider refresh and calendar choice must still be completed on the coordinator Mac. Actual account and physical-phone acceptance are still pending.
