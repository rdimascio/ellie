...
The model-context module supplies the exact supported action shapes, field limits, temporal formats and optional fields. The contract covers replies, source search, explicit memory, reminders, events, needs, contacts, saved checklists, agenda queries and source summaries. A plan has at most eight actions and at most one mutation-family proposal, including a source-summary workflow or an incomplete action draft.
...
Saved checklists use `create_plan` with a title of at most 200 characters and one through 24 text steps (500 characters per step, 8,000 combined). The host saves unfinished steps in one revisioned life record and never executes their text. World context supplies total completion progress and up to eight whole numbered step titles, favoring unfinished entries and explicitly counting omitted steps. Oversized titles are omitted whole. Questions about an existing checklist use this read-only state rather than an agenda operation.
