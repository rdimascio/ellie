...
The primary Mac and iPhone interfaces are SwiftUI applications. Main now contains the [native Mac dashboard](native-app.md), local chores and opt-in weather, with browser-export import compatibility. The release branches add native Devices, iPhone enrollment and granted controls, household synchronization and local voice. Their implementation and acceptance status is tracked separately in the [delivery queue](delivery-queue.md); the active priority is reliable signed distribution and combined release validation. Existing app-opening credentials do not inherit household data access: see the [explicit authority contract](native-household-state.md).
...
4. **Native household clients:** connect native SwiftUI Mac and iPhone views to authenticated APIs in vertical slices: health and explicit node selection, granted commands, shared layouts, agenda and routines. Preserve reviewed protocol behavior and dashboard import/export compatibility. Use Keychain-backed pairing and explicit microphone consent, with shared models independent from platform views. The TV role remains read-only by default; its presentation surface is a separate decision.
