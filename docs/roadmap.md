...
The primary Mac and iPhone interfaces are SwiftUI applications. The development source now includes the [native Mac dashboard](native-app.md), chores, opt-in weather, selected playlists, offline and selected Google calendar views, Devices and pairing management, iPhone enrollment and granted controls, explicitly shared chores, local voice and read-only Gmail views. Main includes companion packaging and native pinned-HTTPS acceptance from PR175–177, plus the public YouTube popup authority correction in PR182. The integrated native phone, Quiet, Watch and YouTube workflow in PR180 and its app-hosted chores/Quiet verification in PR185 are pending merge and current-source checks. These are source milestones, not installed household acceptance. The browser prototypes remain compatibility and protocol references. Current status and the next attended checks are in the [delivery queue](delivery-queue.md). Existing app-opening credentials do not inherit household data access: see the [explicit authority contract](native-household-state.md).
