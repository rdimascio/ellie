...
The primary Mac and iPhone interfaces are SwiftUI applications. The development source now includes the [native Mac dashboard](native-app.md), chores, opt-in weather, selected playlists, offline and selected Google calendar views, Devices and pairing management, iPhone enrollment and granted controls, explicitly shared chores, local voice and read-only Gmail views. Twenty PRs158–177 are merged, including native Gmail, Netflix row navigation/search, scoped YouTube TV controls, companion packaging and synthetic native HTTPS acceptance. Subsequent voice, Watch, YouTube, sync and CI candidates still require their exact combined checks. These are source milestones, not installed household acceptance. The browser prototypes remain compatibility and protocol references. Current status and the next attended checks are in the [delivery queue](delivery-queue.md). Existing app-opening credentials do not inherit household data access: see the [explicit authority contract](native-household-state.md).
