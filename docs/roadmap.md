...
The product UI is now native-first: SwiftUI on macOS, followed by a native iPhone client. The browser prototypes remain compatibility and protocol references. The [initial native Mac app](native-app.md) implements local dashboards, clocks, notes, and browser-export import; live coordinator controls and shared household data are the next client slices. See [its validation record](validation/2026-09-13-native-app.md).
