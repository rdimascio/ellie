...
The primary Mac and iPhone interfaces are SwiftUI applications. The development source now includes the [native Mac dashboard](native-app.md), chores, opt-in weather, selected playlists, offline and selected Google calendar views, Devices and pairing management, iPhone enrollment and granted controls, explicitly shared chores, local voice and read-only Gmail views. Main includes companion packaging and native pinned-HTTPS acceptance from PR175–177, the integrated native phone, Quiet, Watch, YouTube and shared-sync workflow from PR180, and the public YouTube popup authority correction in PR182. PR180 passed its required checks before merging; PR185 adds app-hosted chores/Quiet verification and remains pending merge and current-source checks. These are source milestones, not installed household acceptance. The browser prototypes remain compatibility and protocol references. Current status and the next attended checks are in the [delivery queue](delivery-queue.md). Existing app-opening credentials do not inherit household data access: see the [explicit authority contract](native-household-state.md).
...
A separate, opt-in **Ellie Life** web client exercises conversational memory, teaching, background work and generated applications against persistent local stores; see its [implementation checkpoint](overnight-build.md) and [runbook](life-runbook.md). Source now includes narrowly granted native read-only agenda and Gmail clients, with app-hosted pinned-HTTPS tests against synthetic providers. This does not establish an installed native Life session or real Google account consent.
...
A browser phone/TV prototype remains available for compatibility testing, and QR pairing plus the opt-in native phone voice-to-Open-Arc subset have user-reported physical acceptance. The native clients and explicitly granted household synchronization are development features. PR182's account-free genuine-popup run searched public YouTube, scrolled once, selected one observed long-form result and freshly read a visibly playing video without a synthetic AX seam. Separately, later source `e2415bc` passed native Search, observed selection and exact-video read in an owned Chrome for Testing browser with synthetic auth/AX seams; playback was unavailable and no native Play action passed. The physical phone-to-public-site route remains unaccepted. The Watch source successor `244dad2` passed three paired Simulator cases against a synthetic phone backend; actual phone-outage recovery and physical Watch control remain pending. Ellie Life adds local scoped routines, reminder schedules, reviewed source imports, task orchestration and versioned extensions. Packaged-service tooling can build and inspect development payloads; the current successor payload is prepared, not installed. Authenticated capture remains unselectable and cannot authorize installation or launch. Production signing, notarization, installed-service acceptance, real Google consent, the full phone-to-website journey, a TV application, an MCP server, optional external analytics and automatic updates remain pending. The development workflow uses Bun for package management and scripts, Oxlint and Oxfmt for source checks, TypeScript for type checking, Node.js 24 for server and CLI execution, and Swift for native applications, macOS Accessibility and Keychain integration.
