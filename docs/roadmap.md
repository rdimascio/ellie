...
The maintained [delivery queue](delivery-queue.md) now drives ongoing work on editable widget dashboards, full local voice, Gmail/Calendar, weather, chores/charts and selected YouTube playlists. Product interfaces use native SwiftUI on macOS and iPhone. The browser prototype remains a compatibility and protocol reference, including version 1 dashboard import/export. Independently reviewed native slices provide local dashboards, chores, weather and bounded command controls; they are not yet an integrated household release. The queue records implementation boundaries, next ready slices and external acceptance gates. [iPhone testing](iphone-testing.md) separates browser automation from physical camera/microphone and trust setup; the native iPhone target has its own isolated simulator workflow.
