...
`resume` restores a paused schedule template to `scheduled` with its original next occurrence; the next tick applies the template's persisted missed-run policy. `runNow` on a scheduled or paused template materializes one child occurrence at the current time without shifting or completing the template; a paused template remains paused. For ordinary nonscheduled work, `runNow` advances only active queued, scheduled, paused, or waiting work. It rejects terminal tasks rather than implying that completed work was repeated. Cancelling a terminal one-shot template leaves its verified terminal result intact but still cancels queued or running descendants. A product-level rerun of terminal work creates a new task or workflow with fresh inputs, authority checks, idempotency keys, and deadlines.

`deliveryOccurrences(templateId, owner)` reads direct children in SQL and returns the newest nonterminal occurrence plus the newest occurrence overall. It does not scan or truncate occurrence history, and an owner mismatch returns no rows. Product delivery status can therefore distinguish the template's future schedule state from the current or latest occurrence outcome.
