...
Omit `--node ACTUAL_ID` when exactly one eligible node is online. The command prints the desktop effect before submitting it. An unknown ID, a paired node that is offline, and a registered node whose heartbeat is stale produce different guidance. The ordinary `say` command uses the same single-node selection when it runs on a coordinator-only Mac; use `say --node ACTUAL_ID` when several execution nodes are online.
