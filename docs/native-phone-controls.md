...
For YouTube, the selected Mac's fresh companion observation must likewise expose one reviewed search field on a home or results page. The iPhone names that field before Run and does not infer that an unverified Search succeeded. A new read must identify the results document and its title handles before selection. The `browser.read` and `browser.control` grants remain separate; if the observed companion control is unavailable, ordinary scoped Accessibility reading remains available without advertising Search.
