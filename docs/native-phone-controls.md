...
- `POST /native/v1/commands`: exact `{nodeId, action}` with `action` restricted to the closed `app.open`, `browser.status`, `browser.read`, `browser.scroll`, `browser.search`, `browser.select` and `browser.playback` schemas. App opening accepts only `arc`, `safari` and `messages`. The operation's required native capability is checked before discovery, after discovery and at dispatch.
