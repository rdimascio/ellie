...
## Bounded browser pursuit

`ellie browser pursue --node ID --max-steps N --allow-page-content "goal"` repeats a small loop on the CLI controller: `browser.status`, `browser.read`, a proposal from `POST /v1/decisions/browser-step` on the coordinator, then at most one `browser.select` or `browser.scroll` dispatched through the ordinary `/v1/commands` one-action contract. `--max-steps` (1-8) has no default and `--allow-page-content` must be passed every invocation; it discloses that page titles, summaries, and item labels leave this Mac for the decision provider, beyond the command text `say` already sends. The loop only runs against an already-authorized browser binding a human selected; it cannot create one, and it never emits `browser.search` or `browser.playback` since Jev answers bounded choices, not free text.

The endpoint asks two bounded questions per step: `progress` (satisfied/unsatisfied/blocked against the stated goal) gates first, and only `unsatisfied` proceeds to `step` (one observed page item, an observed scroll direction, `done`, or `cannot`). `progress: satisfied` stops the loop early; it never upgrades a dispatched action to a confirmed outcome. Every dispatched step is reported `unknown` forever, and the final report says so explicitly. Outcomes are `satisfied_unverified`, `blocked`, `ambiguous`, `exhausted`, `no_change`, `cancelled`, `unavailable`, or `shadow`; a run exits 0 only when nothing was dispatched and the goal was already judged satisfied.

The loop reuses this document's existing `minProbability`/`minMargin` gate unchanged; there is no separate browser threshold to tune. A real page can offer dozens of selectable elements, and 0.98/0.2 applied to a many-way choice is expected to abstain often — that is the gate working, not a bug to route around. Lowering these thresholds to make a click succeed would also loosen desktop `say` routing, since both paths share the same knobs. A wall clock of 120 seconds and no repeated `(revision, itemId)` dispatch bound the loop further; an unchanged page after a dispatch (same title and sorted item labels, not ids, since a fresh id is minted per read) stops the loop instead of clicking again.
