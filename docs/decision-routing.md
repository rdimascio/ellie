...
Ellie can interpret unmatched desktop requests through a replaceable decision provider. Known grammar commands still use the existing deterministic path. This experiment covers the four existing desktop operations only: opening configured apps/sites, placing windows, and adjacent placement. It does not add general conversation, memory storage, planning, new tools, or background watches.
...
The checked-in dataset contains only synthetic commands, with development and held-out splits. It labels complete action arguments as well as requests that should abstain. Start with the offline deterministic baseline:
...
`@ellie/decisions` defines Choice, Score, and Noul questions with validated responses. `@ellie/router/decision` converts these judgments into bounded desktop plans. Future memory matching, source ranking, notification relevance, and plugin selection can reuse the provider boundary when their underlying stores and permissions exist. Each should get a narrow question set, code-owned policy, and its own labeled evaluation before activation.
