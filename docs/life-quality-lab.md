...
`LIFE_QUALITY_SCENARIOS=planning` selects a separate real-model scenario; it requires both model variables and is excluded from the five-case synthetic default. It starts with an empty Plans view, submits an ordinary two-step checklist request through the orb, verifies one actual saved plan, completes the first checkbox, then reloads and reopens the same plan. Both step IDs, the remaining unfinished step and the increased record revision must survive. Store reads only corroborate the UI; the runner never creates or completes a plan through a fixture API. On small screens it scrolls the card clear of the floating orb before clicking.

`LIFE_QUALITY_PLANNING_VARIANT=compound` is the default request with an introductory sentence. Select `direct` for the second ordinary request form. Both use the same saved-plan and completion oracle, and the report records the exact prompt and variant. Other values fail before fixture startup. Run each trial with a unique report directory and fresh fixture; preserve failures instead of repeatedly overwriting one successful report.

Loopback runs retain bounded outbound model requests in private `model-requests.json`, including a repair request when used, even if the scenario fails. `model-results.json` records the validated plan or terminal model error. `model-responses.json` retains up to 32 KiB of wire data for each of the first 64 responses, with explicit truncation and completion metadata. Capture forwards the original bytes under consumer demand and propagates cancellation; the model adapter normally cancels an SSE stream after receiving `[DONE]`, so that capture outcome alone does not imply truncation. The report distinguishes total captures from omitted entries. Request count includes repairs and is not the number of submitted user turns. This evidence establishes what reached the model; browser snapshots and persisted records establish the result. A prose checklist without a saved plan fails acceptance.

`LIFE_QUALITY_SCENARIOS=real-memory` selects a separate opt-in scenario requiring the same explicit model configuration. Run it alone against its fresh fixture. It submits four turns through the orb: an ordinary preference, recall in a new conversation, Forget in that conversation, and a neutral preference question in a third conversation. The three modeled turns allow at most one repair each; deterministic Forget must use zero model calls. It verifies the original automatic-memory row changes from selectable to suppressed, every final outbound message and the latest assistant answer omit the forgotten detail, and the original chat remains visible when reopened through History. Store reads corroborate the actual UI actions; no preference or suppression is injected into fixture state.

The memory evidence retains exact replies for independent semantic review. Its automated recall checks establish that the answer mentions the preference and its timing, and that the complete original statement reaches system context. They do not prove the answer preserves the preference's scope: “any tea suggestions, especially after lunch” broadens “after lunch” even though both pass the lexical check. Review qualification drift and any false assertion that the user never shared the detail separately. Forget excludes the suppressed fact from later model context; it does not physically delete the original journal or visible transcript. This scenario does not establish process-restart behavior or general model reliability.
