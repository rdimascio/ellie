...
Run `bun run ellie browser init` on the coordinator Mac to prepare this separate identity. The command reads the Mac's current LocalHostName through `scutil`, reserves port 8444 in browser configuration, stores the CA and server private keys under the distinct Keychain accounts `browser-ca-key` and `browser-server-key`, and writes only their public certificates and metadata to private Ellie state. It is idempotent and never repairs, replaces, or rotates partial or mismatched state. `bun run ellie browser status` reports certificate, key, permission, expiry, and LocalHostName-drift problems without printing private keys. These commands do not start or bind a listener.

The browser listener is not wired yet. A future runtime change must acquire the existing coordinator lifetime lock before it owns browser authentication and port 8444. Failure of that optional listener must leave the certificate-pinned agent coordinator running. Runtime loading validates the persisted public configuration and certificate chain before retrieving only the server leaf key; it does not load the CA key or rotate a changed hostname or certificate.

Before first use, build the current native helper explicitly with `bun run build:macos`; an older installed helper does not have the add, presence-check, and rollback operations required by browser initialization. This build and its Keychain prompts remain an operator action. Browser initialization serializes concurrent attempts and rolls back only new browser artifacts that its own attempt created. If it reports incomplete cleanup or partial state, preserve the files and Keychain items and inspect `browser status` before recovery; do not rerun initialization as a rotation mechanism.

Export the public trust anchor with `bun run ellie browser export-ca /absolute/path/ellie-browser-ca.pem`. The command refuses to overwrite an existing path unless `--force` is explicitly supplied. It exports only the public CA certificate and does not install or trust it. Transfer it through an existing trusted local channel, then follow the platform trust steps above. A native macOS Security.framework check on a separate Mac accepted the generated leaf for its exact hostname and rejected the same leaf for another DNS name and an IP address using per-evaluation anchors; it made no trust-store change or network request. Safari, iPhone, iPad, and Apple TV bootstrap and trust remain unvalidated.
