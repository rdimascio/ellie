...
1. Server initialization generates a self-signed TLS certificate with a generic identity. The private key is captured in process memory and stored through the native Keychain helper, never written to a private-key file.
