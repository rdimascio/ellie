...
Browser authorization has a separate versioned state model with fixed phone-controller and read-only TV roles. Invitations and sessions use independent 256-bit bearer values, while the private server file stores only their SHA-256 verifiers. Invitation role, label, target and operation-capability grants are fixed before pairing. TV identities cannot carry operation grants. Pairing, revocation and logout publish their result only after the corresponding private state update succeeds. See [Browser session foundation](browser-sessions.md) for the implemented limits and the still-pending trusted-HTTPS boundary.
