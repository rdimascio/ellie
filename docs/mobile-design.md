...
The current integration preserves this treatment on the dedicated Integrations screen while carrying forward the newer connected-account workflow. Google setup tracks the exact pending connection and offers Stop setup; a connected calendar can choose one calendar to read, and connected Calendar or Gmail activity has a private read-only preview with sync and error status. Polling, account switching, and revocation still invalidate stale details. The default Prepare checkbox remains explicit. The authenticated owner settings link remains available alongside mobile Integrations navigation. Multiple simultaneous calendar selections and live Google account acceptance remain pending.
